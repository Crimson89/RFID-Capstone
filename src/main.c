...
char* check_125(char* cardNum)
{
    if(cardNum != NULL)
    {
      ...
      int ret = 1;
      int Led_Pin = 0;
      //ret = http request thingy pass cardNum and station id
      if (ret == 1)
      {
        // turn on green led
	      Led_Pin = 4;
...
int main(/*int argc, char *argv[]*/) {
  ...
  //printf("Starting 125 KHz reader\n");
  //pthread_create(&thread_id1, NULL, &read_125, &cardNo125);
  //printf("Starting 125 KHz checker\n");
  //pthread_create(&thread_id2, NULL, &check_125, &cardNo125);
